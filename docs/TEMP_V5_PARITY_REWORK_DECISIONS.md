...
Status: approved requirements implemented; final live verification pending

Last consolidated: 2026-07-18

This is the detailed decision log and implementation contract for the V5 UI,
behavior, audio, and QA rework. T0040 implements the approved decisions below;
individual release claims remain subject to the final evidence manifest and
...
## Superseded baseline and implemented replacement

### Retired audio baseline

The pre-T0040 repository contained only:
...
That one-track/one-general-effect behavior is retired and is not the approved target.
...
- The approved 12-track library is intentionally looped for a 3:10:00 exam. Shuffle uses a bag that plays every track once before refilling and prevents the same track from playing twice in succession. No unsupported unique-duration claim is made.
