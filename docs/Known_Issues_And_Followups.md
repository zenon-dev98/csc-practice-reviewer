...
- T0041's automated suite validates camera-style files, local image handling,
  OMR normalization, frozen timeout, and the complete grade path. A physical
  rear-camera permission/capture on a phone and a real two-device Supabase
  handoff still require hardware/account verification; the app exposes both
  paths, but they are not claimed as manually certified by desktop automation.
